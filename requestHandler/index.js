const handleRequest = (state, io, req) => {
  ...
  switch(req.type) {
    ...
    case 'new_user': {
      ...
      socket.emit('data', { type: 'update_state', users: session.users });
      break;
    }
    case 'remove_user': {
    ...
    case 'set_round_length': {
      ...
      socket.emit('data', { type: 'update_state', roundLength: session.roundLength });
      break;
    }
    case 'toggle_skip_user': {
      ...
      socket.emit('data', { type: 'update_state', users: session.users, currentTime: session.currentTime, currentUser: session.currentUser })
      break;
    }
